...
@section('content')
    <div class="max-w-4xl mx-auto card p-8">
        ...
        <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data" x-data="{ submitted: false }" @submit="submitted = true">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end gap-3 mt-8">
                ...
                <button type="submit" class="btn-primary py-3 px-6 text-sm flex items-center justify-center gap-2" :disabled="submitted" :class="submitted ? 'opacity-75 cursor-not-allowed bg-primary-rose/80' : ''">
                    ...
                    <span x-text="submitted ? '{{ __('Processing...') }}' : '{{ __('Update Book') }}'"></span>
                </button>
            </div>
        </form>
    </div>

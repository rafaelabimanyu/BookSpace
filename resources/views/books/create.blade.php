        ...
        <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-2">
                    ...
                    <input type="file" name="cover_image" id="cover_image" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-secondary-blush file:text-primary-rose hover:file:bg-primary-rose hover:file:text-white file:transition file:cursor-pointer cursor-pointer border border-secondary-blush/60 rounded-2xl p-2 bg-white" accept="image/*">
                    @error('cover_image')
                        <p class="text-red-500 text-xs mt-2 font-medium">{{ $message }}</p>
                    @enderror
                </div>
